...
class RemoveTeamMemberHandler(CommandHandler[TeamDTO]):
    def __init__(
        self,
    ...
    async def handle(self, command: Command) -> TeamDTO:
        assert isinstance(command, RemoveTeamMember)
        ...
        await self._team_repo.remove_member(command.team_id, command.user_id)
