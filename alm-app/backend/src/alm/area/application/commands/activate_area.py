...
class ActivateAreaNode(Command):
    tenant_id: uuid.UUID
    project_id: uuid.UUID
    area_node_id: uuid.UUID


class ActivateAreaNodeHandler(CommandHandler[AreaNodeDTO]):
    def __init__(
        self,
    ...
    async def handle(self, command: Command) -> AreaNodeDTO:
        assert isinstance(command, ActivateAreaNode)
