"""Set area node is_active = False (soft delete)."""
...
class DeactivateAreaNode(Command):
    tenant_id: uuid.UUID
    project_id: uuid.UUID
    area_node_id: uuid.UUID


class DeactivateAreaNodeHandler(CommandHandler[AreaNodeDTO]):
    def __init__(
        self,
    ...
    async def handle(self, command: Command) -> AreaNodeDTO:
        assert isinstance(command, DeactivateAreaNode)
