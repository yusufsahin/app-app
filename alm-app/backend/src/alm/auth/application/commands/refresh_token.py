...
class RefreshTokenHandler(CommandHandler[TokenPairDTO]):
    ...
    async def handle(self, command: Command) -> TokenPairDTO:
        ...
        new_raw = self._token_service.create_refresh_token_value()
    ...
    async def _resolve_tenant_id(self, user_id: uuid.UUID, hint: uuid.UUID | None) -> uuid.UUID:
        if hint is not None:
            return hint
        ...
        raise ValidationError("Multiple tenants found. Please provide tenant context or re-authenticate.")
