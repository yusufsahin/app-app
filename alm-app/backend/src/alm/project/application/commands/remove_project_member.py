...
class RemoveProjectMemberHandler(CommandHandler[bool]):
    ...
    async def handle(self, command: Command) -> bool:
        ...
        member = await self._project_member_repo.find_by_project_and_user(
        ...
        if member.role == "PROJECT_ADMIN":
            admins = [
                m for m in await self._project_member_repo.list_by_project(command.project_id)
        ...
        deleted = await self._project_member_repo.delete_by_project_and_user(
            command.project_id, command.user_id
        )
        return deleted
