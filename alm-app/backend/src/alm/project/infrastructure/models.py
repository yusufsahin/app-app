...
class ProjectModel(Base, TimestampMixin, SoftDeleteMixin):
    ...
    process_template_version_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=True, index=True)
    status: Mapped[str] = mapped_column(String(50), nullable=True)
    settings: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=True)
    metadata_: Mapped[dict[str, Any]] = mapped_column("metadata", JSONB, nullable=True)
