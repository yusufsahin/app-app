...
async def test_transition_artifact_optimistic_locking_conflict():
    ...
    # Act & Assert
    with patch("alm.artifact.application.commands.transition_artifact.get_manifest_ast", return_value=simple_manifest_ast()):
...
async def test_transition_artifact_invalid_trigger():
    ...
    # Act & Assert
    with patch("alm.artifact.application.commands.transition_artifact.get_manifest_ast", return_value=simple_manifest_ast()):
        with patch("alm.artifact.application.commands.transition_artifact.get_permitted_triggers", return_value=[]):
            with pytest.raises(ValidationError, match="is not permitted"):
                await handler.handle(command)
