...
**S1+ (MVP’ye yakın, düşük sürtünme):** “Add link” akışında **yapıştırılan PR/commit URL’sinden** `owner/repo`, PR #, kısmi SHA alanlarını doldur (Faz S2 parse mantığının erken sürümü).
...
- URL’den `owner/repo`, PR #, commit kısmi parse (GitHub/GitLab bilinen pattern’leri); başlık/açıklama metninden **`artifact_key`** (ve varsa task kimliği) çıkarma.
...
- **Deterministik öncelik sırası (öneri):** (1) branch adı → (2) PR başlığı → (3) PR açıklaması → (4) son ilgili commit’in footer’ı. Çakışmada ilk başarılı eşleme veya proje politikasına göre en spesifik kural (dokümante edilmeli).
...
- Artifact başına ortalama SCM bağlantı sayısı; webhook’tan otomatik oluşturulan kayıt oranı (Faz S3 ve sonrası); **manuel vs otomatik** kayıt dağılımı.
- **Unmatched webhook** sayısı / oranı ve triage **çözüm süresi** (Faz S3).
