...
- [x] `traceability_graph.yaml` pilot için `artifact_link` ile uyumlu (veya export script’i yazıldı) — statik graf repo’da; DB senkronu için `scripts/export_traceability_graph.py` + `governance_artifact_id` eşlemesi (`docs/ARTIFACT_GOVERNANCE.md`).
...
- [x] `artifact_link` → `traceability_graph.yaml` export — `scripts/export_traceability_graph.py`; demo seed iki yönlü link + üç `governance_artifact_id` ile yerel doğrulama için veri üretir (`seed_demo_data`).
