...
> **Sonraki evrim (MPC hizalama):** Grafik motoru tekrar manifest üzerinden **`mpc.features.workflow.WorkflowEngine`** (`workflow_sm`) ile besleniyor; `TransitionPolicy` için `mpc_resolver.check_transition_policies`, `Policy` kind için `PolicyEngine` kullanılıyor. Bu not tarihsel özet için korunmuştur; güncel sınır için [WORKFLOW_ENGINE_BOUNDARY.md](../WORKFLOW_ENGINE_BOUNDARY.md).

## Summary

- **Tek giriş noktası (`workflow_sm`):** Geçiş grafiği ve permitted triggers bu modülden yönetilir (güncelde MPC `WorkflowEngine` ile; bu release notu yazıldığında statelesspy odaklı anlatım geçerliydi).
...
- **MPC_BOUNDARY.md / WORKFLOW_ENGINE_BOUNDARY.md:** MPC–ALM sınırı; sonraki commit’lerde MPC `WorkflowEngine` + `check_transition_policies` ile güncellendi.
