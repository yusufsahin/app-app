...
| **Planning** | Cadence (release/cycle), area, baseline | Manifest veya ayrı planning config; alan/cadence path metadata |
...
- Cadence (release/cycle), area, baseline kavramları.  
- Hangi alanların (area path, cadence path vb.) hangi entity’lerde olduğu **manifest veya planning metadata** ile tanımlı.  
- List/board görünümleri bu metadata’ya göre (filtre, gruplama).

**Çıktı:** Planning context metadata-driven; release/cycle/area manifest veya config ile genişletilebilir.
