...
- **Önce** `mpc_resolver.check_transition_policies(ast, event)` (TransitionPolicy; assignee required vb.).
...
- **MVP:** `GET /projects/{id}/velocity?cycle_id=...&last_n=5` (veya çoklu `cycle_id`): her cycle için `state=done` (veya workflow’daki “terminal” state’ler) artifact’ların effort toplamı. Cycle = Sprint olarak kullanılıyor (CycleNode).
...
- **Backend:** Velocity ile aynı effort alanı; `GET /projects/{id}/burndown?cycle_id=...` (tarih bazlı remaining/completed noktaları).
...
**Sprint ayrı entity değildir; Cycle olarak implemente edilmiştir.** ALM'de planlama zaman ekseni `Cadence` domain modeliyle temsil edilir; UI'da bunun child seviyesi `Cycle` olarak görünür. Teknik olarak `alm/cycle` modülü release/cycle ağacını taşır. Artifact'lar `cycle_id` ile cycle'a atanır; Planning sayfası cycle backlog ve release/cycle/area filtreleri ile mevcut. Bu plan kapsamında Sprint ayrıca “out of scope” sayılmaz — Cycle/CycleItem (artifact'ın cycle'a atanması) zaten scope içindedir.
...
- **Veri:** Velocity ile aynı effort alanı + cycle_id + state; “remaining” = cycle’a atanmış ama henüz done olmayan artifact’ların effort toplamı (veya toplam planlanan − tamamlanan).
- **Backend:** Cycle bazlı velocity endpoint’ine ek veya ayrı `GET /projects/{id}/burndown?cycle_id=...` (tarih bazlı remaining/completed serisi).
...
1. **D1 (P1):** `build_artifact_transition_policy_event` + `evaluate_transition_policy` (içinde `check_transition_policies` + `PolicyEngine.evaluate`); ihlal → `PolicyDeniedError` / anlamlı mesaj. Unit test: event + örnek manifest.
...
5. **Burndown (P5):** Burndown endpoint (`cycle_id`, tarih serisi); frontend'te burndown grafiği.
