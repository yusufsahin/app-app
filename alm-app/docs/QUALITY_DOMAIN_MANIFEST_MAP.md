...
| Test parameters | **`test_params_json`**: versioned embedded configuration document, currently `v: 2`. Shape: `{ v: 2, defs: [{ name, label?, default?, type?, required?, allowedValues? }], rows?: [{ id, name?, label?, values: Record<string,string>, isDefault?, status?, tags? }] }`. In product language, each row is a named **configuration**. |
| Run/campaign structured extras | **`run_metrics_json`** (`v` + `results`): each result may include **`expandedStepsSnapshot`**, **`configurationId`**, **`configurationName`**, **`configurationSnapshot`**, and **`resolvedValues`**. Legacy `paramRowIndex` / `paramValuesUsed` may still appear on older runs. **`campaign_config_json`** string field (JSON). |
| Workflows | **`quality_run`**, **`quality_campaign`** for run/campaign states; suite uses the same workflow as `test-case` per template (`basic`, `scrum`, `kanban`, `ado_basic`). **`quality_folder`** — `Planning` / `Active` / `Archived` for **`quality-folder`** (catalog groups) and **`testsuite-folder`** (campaign collections) so Board/Kanban is not a single column. |
...
- UI: Quality hub + `/quality/catalog` (Catalog), `/quality/campaign` (Campaign workspace), `/quality/runs`, `/quality/campaigns` (`test-campaign` items).
